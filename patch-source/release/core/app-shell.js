...
async function renderRoute() {
  ...
  if (screen === 'patient') {
    ...
    setEncounterIdentityLock(Boolean(currentDraftEncounterId));
    renderWizard();
  }
  showScreen(screen);
}
...
function sameMonth(value, date = new Date()) {
  ...
  return !Number.isNaN(candidate.getTime()) && candidate.getFullYear() === date.getFullYear() && candidate.getMonth() === date.getMonth();
}
function renderHomeHeader(now = new Date()) {
  const dateText = new Intl.DateTimeFormat('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' }).format(now).toUpperCase();
  setText('[data-home-date]', dateText);
  const greeting = now.getHours() < 12 ? 'Bom dia' : now.getHours() < 18 ? 'Boa tarde' : 'Boa noite';
  setText('[data-home-greeting]', `${greeting}, Débora`);
  const todayCount = state.appointments.filter((a) => new Date(a.starts_at).toDateString() === now.toDateString()).length;
...
function renderDateStrip(now = new Date()) {
  ...
  root.innerHTML = Array.from({ length: 7 }, (_, index) => {
    ...
    return `<button type="button" class="date-pill ${selected ? 'active' : ''}" ${selected ? 'aria-current="date"' : ''}><span>${escapeHTML(weekday)}</span><strong>${date.getDate()}</strong></button>`;
  }).join('');
}
function nextAppointment(now = new Date()) {
  return state.appointments.filter((a) => new Date(a.starts_at) >= now && a.status !== 'Cancelado').sort((a,b) => new Date(a.starts_at)-new Date(b.starts_at))[0] || null;
}
function renderAgenda() {
  const root = document.querySelector('[data-agenda-live]');
  const homeRoot = document.querySelector('[data-home-agenda-live]');
  const now = new Date();
  renderHomeHeader(now); renderDateStrip(now);
  const today = state.appointments.filter((a) => new Date(a.starts_at).toDateString() === now.toDateString());
  const sorted = [...state.appointments].sort((a,b) => new Date(a.starts_at)-new Date(b.starts_at));
  const render = (items) => items.length ? items.map((a) => {
    const p = patientByMotherId(a.mother_id) || patientByBabyId(a.baby_id);
    return `<article class="agenda-card"><time>${escapeHTML(timeLabel(a.starts_at))}</time><div class="agenda-main"><strong>${escapeHTML(p ? patientTargetLabel(p, a.baby_id) : 'Paciente')}</strong><span>${escapeHTML(`${a.appointment_type || 'Atendimento'} · ${a.format || ''}`)}</span></div><span class="pill ${a.status === 'Realizado' ? 'completed' : a.status === 'Confirmado' ? 'confirmed' : 'waiting'}">${escapeHTML(a.status || 'Agendado')}</span><div class="agenda-actions"><button data-action="open-patient" data-patient-id="${escapeHTML(a.mother_id)}">Abrir</button></div></article>`;
  }).join('') : '<div class="empty-live">Nenhum atendimento nesta lista.</div>';
  if (root) root.innerHTML = render(sorted);
  if (homeRoot) homeRoot.innerHTML = render(today.slice(0, 3));
  ...
  if (next) {
    ...
    setText('.next-patient strong', p ? patientTargetLabel(p, next.baby_id) : 'Paciente');
    setText('.next-patient span', `${next.appointment_type || 'Atendimento'} · ${next.format || ''}`);
    if (nextCard) { nextCard.dataset.motherId = next.mother_id || ''; nextCard.dataset.address = next.address || p?.mother?.address || ''; }
    ...
    if (nextCard) { delete nextCard.dataset.motherId; delete nextCard.dataset.address; }
...
async function ensureEncounterStarted() {
  ...
  const startsAt = ident.startsAt ? new Date(ident.startsAt).toISOString() : new Date().toISOString();
...
async function finalizeEncounter() {
  ...
  await appData.updateAppointment(currentAppointmentId, {
    ...
    status: new Date(startsAt) <= new Date() ? 'Realizado' : 'Agendado',
...
async function scheduleAppointment() {
  ...
  await appData.createAppointment({ mother_id: patient.mother.id, baby_id: baby?.id || null, starts_at: new Date(when).toISOString(), duration_min: 60, appointment_type: type, format, status: 'Agendado', value_cents: Math.max(0, Math.round(value * 100)), payment_status: 'Pendente', notes: baby ? '' : `Atendimento conjunto: ${babyNames(patient).join(', ')}` });
