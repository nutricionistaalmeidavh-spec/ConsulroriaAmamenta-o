...
const LABELS={
  ...
  clinical_media:{title:'Fotos, vídeos e documentos clínicos',description:'Registro de autorização para armazenamento privado de fotos, vídeos e documentos vinculados ao cuidado.'},
...
const LEGACY_CLINICAL_MEDIA={title:'Fotos e documentos clínicos',description:'Versão anterior da autorização para armazenamento privado de imagens e documentos vinculados ao cuidado.'};
...
function consentMeta(type,consent=null){if(type==='clinical_media'&&consent?.id&&consent.version!=='1.1')return LEGACY_CLINICAL_MEDIA;return LABELS[type]||{title:type,description:''}}
function refreshClinicalMediaConsentCopy(){
  const input=document.querySelector('input[name="consentClinicalMedia"]'),row=input?.closest('.consent-row');
  const title=row?.querySelector('strong'),detail=row?.querySelector('small');
  if(title)title.textContent='Fotos, vídeos e documentos clínicos';
  if(detail)detail.textContent='Armazenamento privado vinculado ao atendimento. Vídeos exigem novo aceite desta versão.';
}
...
async function ensureSnapshot(context,consent){
  const s=status(consent),meta=consentMeta(consent.consent_type,consent);
...
async function pdfFor(context,consent){
  const meta=consentMeta(consent.consent_type,consent),s=status(consent);
...
async function openConsent(motherId,consentType,trigger){
  ...
  const meta=consentMeta(consentType,consent),s=status(consent);
...
refreshClinicalMediaConsentCopy();
window.addEventListener('debora:patient-context',(event)=>{refreshClinicalMediaConsentCopy();const motherId=event.detail?.motherId;if(motherId)mount(motherId);else{currentMother='';document.querySelectorAll('[data-df-terms-card]').forEach(x=>x.remove())}});
window.DeboraTerms={open:openConsent,refresh:()=>{refreshClinicalMediaConsentCopy();currentMother='';const id=DOC.currentMotherId();if(id)mount(id)}};