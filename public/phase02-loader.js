...
export async function startPhase02(){
  if(phase02Started)return;
  if(!globalThis.DEBORA_APP_CONFIG||!document.querySelector('[data-app-root]')){setTimeout(startPhase02,80);return}
  phase02Started=true;
  try{
    ensureCss();
    await import('./documents-feature.js');
    await import('./terms-feature.js');
  }catch(error){phase02Started=false;console.error('Falha ao carregar documentos clínicos',error);setTimeout(startPhase02,800)}
}
startPhase02();
